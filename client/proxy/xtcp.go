...
func (pxy *XTCPProxy) InWorkConn(conn net.Conn, startWorkConnMsg *msg.StartWorkConn) {
	...
	if err != nil {
		xl.Errorf("P2p运行错误: %v", err)
		return
	}

	xl.Tracef("Nathole 准备启动")
	prepareResult, err := nathole.Prepare([]string{pxy.clientCfg.NatHoleSTUNServer})
	if err != nil {
		xl.Warnf("Nathole 准备失败: %v", err)
		return
	}
	xl.Infof("Nathole 准备成功，NAT 类型: %s, behavior: %s, 地址: %v, %v",
	...
	xl.Infof("P2p打洞成功, sid [%s], 远程地址 [%s]", natHoleRespMsg.Sid, raddr)
