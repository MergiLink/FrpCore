...
func (sv *XTCPVisitor) keepTunnelOpenWorker() {
	...
	for {
		...
		case <-sv.ctx.Done():
			return
		case <-ticker.C:
			xl.Debugf("尝试通过P2p连接服务端")
			conn, err := sv.getTunnelConn()
			if err != nil {
				xl.Warnf("尝试通过P2p连接服务端时发生了错误: %v", err)
...
func (sv *XTCPVisitor) makeNatHole() {
	...
	if err != nil {
		xl.Warnf("NAT打洞准备错误: %v", err)
		return
	}
	xl.Infof("P2p打洞准备成功, NAT类型: %s, behavior: %s, 远程地址: %v, 本地地址: %v",
	...
	xl.Infof("获得NAT打洞信息, 服务ID [%s], 协议 [%s], 远程地址 %v, 本地地址 %v, 动作: %+v",
	...
	xl.Infof("建立NAT连接成功, sid [%s], remoteAddr [%s]", natHoleRespMsg.Sid, raddr)
